...
monitor::monitor(IntelCPU& cpu) : cpu_(cpu) {}

int monitor::run()
{
    int ret = 0;
    cout << endl << "CP/M paralell Monitor V 1.0" << endl << "enter ? for help." << endl;
    do
    {
        cout << endl << "#:" << flush;
        ...
        while (input)
        {
            ...
            case '?':
                cout << endl
                     << "X                     ; exit programm." << endl
                     << "D [hex-addr [bytes]]  ; Dump" << endl
                     << "S [n MHz]             ; set/get Speed" << endl;
                break;
            case 'X':
                return 0;
            case 'S':
