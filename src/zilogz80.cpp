 ...
 *
 * MANY THANKS TO Grant Searle. Thanks a lot for your wonderful documentation about ZX80 video
 * interface. Visit his grate electronic Suite at http://searle.hostei.com/grant or follow him at
 * Twitter by using @zx80nut
 *
...
IntelCPU::IntelCPU()
    : Z80(), m_trace(false), m_reset(0), m_FDCD(-1), m_FDCT(0), m_FDCS(0), m_FDCOP(0), m_FDCST(0),
      m_DMA(0), m_DPH(0), m_BREAK(0)
{
}
...
void IntelCPU::initBIOS(void)
{
    ...
    {
        LOG(LWRN) << "Error extern BOOT-Sector " << BOOTSECTOR << ". Using build in." << e.what()
                  << endl;
...
unsigned IntelCPU::selectdisk(unsigned id, const dpb& dpb, const uint8_t* tltbl)
{
    ...
    // drive id 0=A,1=B...
	extern string diskspath;
	char l='A'+id;
    string drive = diskspath+'/'+l;
    if (id < MAXDRIVE && (id < 4 || id == 7 || id == 8))
    {
        if (id != currentdrive)
        {
			...
			if (stat(drive.c_str(), &st))
            {
				const char *err=strerror(errno) ;
                LOG(LERR) << " Can't stat " << drive << " : " << err << endl;
                return 1;
            }
            ...
            if (!diskptr[id])
            {
                ...
                {
                    diskptr[id] = new disk(drive.c_str(), CPM22, dpb, tltbl);
                }
                else if ((st.st_mode & S_IFMT) == S_IFREG)
                {
                    diskptr[id] = new dskimage(drive.c_str(), CPM22, dpb, tltbl);
                }
                else
                {
                    cerr << drive << " not a file or directory" << endl;
                    return 1;
                }
            }
            else
            ...
            LOG(LINF) << "init disk " << id << ", " << drive
                      << (char*)(diskptr[id] ? " OK" : " ER") << endl;
        }
        return 0;
    }
...
void IntelCPU::readmem(uint8_t* buffer, uint16_t addr, uint16_t size)
{
    ...
    while (size)
    {
        const size_t n = min(static_cast<size_t>(0x10000u - addr), static_cast<size_t>(size));
...
ostream& IntelCPU::readmem(ostream& f, uint16_t addr, uint16_t& size)
{
    ...
    while (f && size)
    {
        const size_t n = min(static_cast<size_t>(size), static_cast<size_t>(0x10000u - addr));
        f.write(reinterpret_cast<const char*>(m_ram + addr), static_cast<std::streamsize>(n));
        if (!f)
            break;
        addr = static_cast<uint16_t>(addr + n);
        size = static_cast<uint16_t>(size - n);
...
istream& IntelCPU::writemem(istream& f, uint16_t addr, uint16_t& size)
{
    ...
    {
        const size_t n = min(static_cast<size_t>(size), static_cast<size_t>(0x10000u - addr));
        f.read(reinterpret_cast<char*>(m_ram + addr), static_cast<std::streamsize>(n));
